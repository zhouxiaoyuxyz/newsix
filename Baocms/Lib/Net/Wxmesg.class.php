...
class Wxmesg{
	...
	static public function pay($data=null){
		...
		return array(
			...
			'data'		   => array(
				...
				'keyword5'=>array('value'=>$data['balance'],'color'=>'#000000'), //变动金额
				'remark'  =>array('value'=>$data['remark'],  'color'=>'#FF3030')//会员账户余额
			)
		);
	}
	//订单取消
	...
	//中奖结果通知
	static public function prize($data=null){
		if(empty($data)) throw new Exception("微信模板消息没有数据！",1002);
		return array(
			'touser'       => '',
			'url'          => $data['url'],
			...
			'topcolor'     => $data['topcolor'],
			'data'		   => array(
				'first'   =>array('value'=>$data['first'],   'color'=>'#e33'),
				'keyword1'=>array('value'=>$data['prize'],   'color'=>'#000000'), //活动奖品
				'keyword2'=>array('value'=>$data['type'],'color'=>'#000000'), //活动名称
				'remark'  =>array('value'=>$data['remark'],  'color'=>'#e33')
			)
		);
	}	
