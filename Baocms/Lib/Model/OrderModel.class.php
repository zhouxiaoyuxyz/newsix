                ...
                if ($order['is_daofu'] == 0) {
                    ...
                    foreach ($goods as $val) {
                        ...
                        if ($money > 0) {
                            ...
                            D('Users')->Money($shop['user_id'], $money, '商户商城订单资金结算：订单ID' . $order_id);
                            //商家结算

                            // 返现到物业费 -- by: zhangdianlei
                            $temp_CONFIG = D('Setting')->fetchAll();
                            $wy_fanxian_money = $temp_CONFIG["site"]["cut"] * $return_cash;
                            $return_cash = $return_cash - $wy_fanxian_money;
                            D('Users')->AddWyFanxian($order['user_id'], $wy_fanxian_money, '抢购返物业费：订单ID' . $order_id);
							...
							D('Users')->addMoney($order['user_id'], $return_cash, '用户返现：订单ID' . $order_id);
                            //用户返现
