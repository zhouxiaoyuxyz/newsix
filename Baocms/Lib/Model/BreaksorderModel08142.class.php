...
class BreaksorderModel extends CommonModel
{
    ...
    public function settlement($order_id)
    {
		...
		//商家获得
		$money = $this->get_discount($shop['shop_id'], $order['amount'], $order['exception']);
        //网站扣除金额，暂时写到购买的会员余额
        ...
        //写入商户资金日志
        D('Shopmoney')->add(array('shop_id' => $order['shop_id'], 'city_id' => $shop['city_id'], 'area_id' => $shop['area_id'], 'branch_id' => $data['branch_id'], 'money' => $money, 'type' => 'breaks', 'order_id' => $logs['order_id'], 'create_time' => NOW_TIME, 'create_ip' => $ip, 'intro' => $intro));
        $this->Breaks_profit(2, $order['user_id'], $order['order_id'], $deduction);
  
        D('Users')->Money($shop['user_id'], $money, '用户买单结算金额，订单号:' . $order['order_id']);
		//用户买单返现
    ...
    public function get_deduction($shop_id, $amount, $exception)
    {
        ...
        //应该计算的金额=消费总额-参与优惠
        if ($shopyouhui['type_id'] == 0) {
            $result_deduction = round($need * $shopyouhui['deduction'] / 10, 2);
            //减去金额=总金额-不参与优惠金额*点数
        } else {
            $t = (int) $need / $shopyouhui['vacuum'];
            //$T是应付款除以网站抽成金额，比如100元，网站抽3元，这里的t就是百分之3
            $result_deduction = round($t * $need / 10, 2);
            //实际付款金额*百分比
        }
	...
	public function get_discount($shop_id, $amount, $exception)
    {
        $shopyouhui = D('Shopyouhui')->where(array('shop_id' => $shop_id, 'is_open' => 1, 'audit' => 1))->find();
        $need = $amount - $exception;
        //应该计算的金额=消费总额-参与优惠
        if ($shopyouhui['type_id'] == 0) {
            $result_discount = round($need * $shopyouhui['discount'] / 10, 2);
            //减去金额=总金额-不参与优惠金额*点数
        } else {
            $t = (int) $need / $shopyouhui['vacuum'];
            //$T是应付款除以网站抽成金额，比如100元，网站抽3元，这里的t就是百分之3
            $result_discount = round($t * $need / 10, 2);
            //实际付款金额*百分比
        }
        return $result_discount*$need;
        //返回网站扣除金额
    }
