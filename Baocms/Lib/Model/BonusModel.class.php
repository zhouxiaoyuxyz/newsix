...
class  BonusModel extends Model{
    ...
    //生成红包
	public function create_bonus($total,$num,$min=0.01)
    { 
		$data['total']=$total;
		$data['num']=$data['remain']=$num;
		$data['min']=$min;
		for ($i=1;$i<$num;$i++)  
		{  
		...
		$bonus[$i] = $total; 
		$data['details']=json_encode($bonus);
	...
	public function sendBonusByUid($uid){
	...
	public function sendBonusByCid($community_id){
        
		static $CONFIG;
		if (empty($CONFIG)) {
			$CONFIG = D('Setting')->fetchAll();
		}
		$total=$CONFIG['profit']['bonus_profit_amount'];
