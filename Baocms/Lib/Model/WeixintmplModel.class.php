...
class WeixintmplModel extends CommonModel{
    ...
    public function weixin_notice_tuan_user($order_id,$user_id,$type){
		...
		//通知用户抢红包
    public function weixin_notice_bonus_user($user_id,$bonusID,$type){
            $Tuanorder = D('Tuanorder')->find($order_id);
		    $Tuan = D('Tuan')->find($Tuanorder['tuan_id']);
			if($type == 'breaks'){
				$pay_type = '邻居买单送' ;
            ...
            include_once 'Baocms/Lib/Net/Wxmesg.class.php';
            $notice_data = array(
				'url' => 'https://' . $_SERVER['HTTP_HOST'] . '/mobile/bonus/index/bonus_id/'.$bonusID.'.html', 
				'first' => '邻居买单红包大派送!', 
				'remark' => '买单送邻居现金红包，不定时推送，先到先得，邀请邻里赚到手软^^^详情登录-https://' . $_SERVER['HTTP_HOST'], 
				 
				'prize' => '转发内页可让身边朋友陪你一起抢红包，会员中心-生成海报:邀请邻里使用惠省活赚取丰厚佣金', 
				
				'type' => $pay_type.'现金红包'
			);
			
            $notice_data = Wxmesg::prize($notice_data);
            Wxmesg::net($user_id, 'OPENTM411968173', $notice_data);
			return true;
    }
    ...
    public function weixin_shop_confirm_refund_user($order_id,$type){
           ...
           $_confirm_refund_data_balance = array(
            ...
            $return = Wxmesg::net($logs['user_id'], 'OPENTM205454780', $confirm_refund_data_balance);
			return true;
    }
    ...
    public function weixin_pay_balance_shop($log_id,$type){
		   ...
		   if($logs['type'] == 'tuan'){
			  ...
			  $num = D('Ordergoods')->where(array('order_id'=>$order_id))->sum('num');
			  $type_name = '商城';  
			  $url = $config_site_url.'mart/all/order_id/'.$logs['order_id'].'/';  
			  $shop_name = $Shop['shop_name'];
			  ...
			  $shop_name = $Shop['shop_name'];
			  $order_goods = '优惠买单';
			  $order_price = '买单金额：'.$Breaksorder['amount'].'元';
           ...
           $_data_order_notice = array(
            ...
            $return = Wxmesg::net($Shop['user_id'], 'OPENTM203353600', $order_notice);
			...
			// 创建SDK对象.
			$sdk = new PushSDK();
			$sdk -> setApiKey('vXeukyaueZ4ItZ0R7ONwou2R');
			$sdk -> setSecretKey('Ya9c7udHkMG9kyt7C8XEfyywO7cQiv6Z');
			$sdk -> setDeviceType(3);

     	//	$channelId = '4115081625611139180';
      ...
      //print $channelId;
            ...
            $order_price1=str_replace("买单金额：","",$order_price);
            try {
                $response = $client->push()
                    ...
                    ->setNotificationAlert("六享互联收款:".$order_price1)
                    ->androidNotification("六享互联收款:".$order_price1, array(
                        'title' => '收款通知!',
                        'builder_id' => 2,
                        //'alert' => "收款".$order_price,
//                        'extras' => array(
//                            'key' => 'value',
//                            'jiguang'
//                        ),
                    ))
                    ->message("六享互联收款:".$order_price1, array(
                        'title' => '收款通知!',
                        // 'content_type' => 'text',
//                        'extras' => array(
//                            'key' => 'value',
...
//                        ),
    ...
    public function weixin_delete_order_shop($order_id,$user_id,$type){
            ...
            $_data_delete_order_notice = array(
             ...
             $return = Wxmesg::net($user_id, 'OPENTM203353600', $data_delete_order_notice);
			 return true;
    }
...
//会员提现，审核，拒绝，通知会员自己
  public function weixin_cash_user($user_id,$tpye){
        ...
        $_cash_data = array(
             ...
             'first'   => $tpye_name,
             'remark'  => '详情请登录-http://'.$_SERVER['HTTP_HOST']."/store/",
             'balance'  => round($Users['gold']/100,2).'元',
             'time'   => $t,
          );
	...
	//打水提醒
	public function weixin_shui_user($user_id,$tpye,$price){
		if($tpye ==1){
			$user_type = '六享会员'; 
		}elseif($tpye ==2){
			$user_type = '押金用户'; 
        ...
        include_once "Baocms/Lib/Net/Wxmesg.class.php";
			$_data_balance = array(
				'url' => "http://".$_SERVER['HTTP_HOST']."/mcenter/logs/moneylogs/",
				'topcolor' => '#F55555', 
				'first' => '尊敬的'.$user_type.'您好，打水成功！', 
				'remark' => '如对上述金额变动有异议，请联系'.$config['site']['sitename'].'客服人员协助处理。' . $config['site']['tel'], 
            ...
            $return = Wxmesg::net($user_id, 'OPENTM205454780', $balance_data);
			//return true;
