...
class CommunityorderModel extends CommonModel {
	...
	public function orderpaid($user_id, $type, $money) {
		...
		if( !empty($products)){
			...
			$log_id = D('Communityorderlogs')->add(array(
                'user_id' => $user_id,
			...
			D('Admin')->Money($community['uid'], $community['community_id'], $money, '物业费结算,缴费订单：'.$log_id);
    ...
    }		public function orderpaidbyweixin($order_id, $user_id, $total) {        $order_id = (int) $order_id;        $detail = $this->find($order_id);        $user_id = (int) $user_id;        $products = D('Communityorderproducts')->where(array('order_id' => $order_id, 'type'=>5))->find();
		$community = D('Community')->find($products['community_id']);        D('Communityorderproducts')->save(array('id' => $products['id'], 'paid' => $products['paid']+$total, 'is_pay' => 2));         $log_id = D('Communityorderlogs')->add(array(                'user_id' => $user_id,                'community_id' => $detail['community_id'],                'money' => $total,                'type' => $products['type'],                'create_time' => NOW_TIME,
                'intro' => '微信缴纳物业费',                'create_ip' => get_client_ip()            ));                   D('Admin')->Money($community['uid'], $community['community_id'], $total, '微信缴物业费,缴费订单：'.$log_id);                return true;    }
