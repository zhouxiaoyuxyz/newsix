...
class UsersModel extends CommonModel{
    ...
    public function getUserByMobile($mobile)
    {
        ...
        $data = $this->where("`mobile` = ".$mobile." OR `ext_mobile` = ".$mobile)->find();
        return $this->_format($data);
    }
     ...
     * @return bool|mixed
     */
    public function AddWyFanxian($user_id, $num, $intro = ''){
        if ($this->updateCount($user_id, 'wy_fanxian', $num)) {
            return D('Userwyfanxianlogs')->add(array(
                'user_id' => $user_id,
                'wy_fanxian' => $num,
                'intro' => $intro,
     ...
     * @return bool|mixed
     */
     public function AddClassifyGold($user_id, $num, $intro = "", $classify_id = 0){
        if ($this->updateCount($user_id, 'classify_gold', $num)) {
            return D('Userclassifygoldlogs')->add(array(
                'user_id' => $user_id,
     ...
     * @return bool|mixed
     */
    public function MoveClassifyGold($user_id, $num, $intro = "", $classify_id = 0){
        if ($this->updateCount($user_id, 'classify_gold', -$num) && $this->updateCount($user_id, 'money', $num)) {

            D('Usermoneylogs')->add(array('user_id' => $user_id, 'money' => $num, 'intro' => '增加:' . $intro, 'create_time' => NOW_TIME, 'create_ip' => get_client_ip()));
            return D('Userclassifygoldlogs')->add(array(
                'user_id' => $user_id,
                'classify_id' => $classify_id,
                'gold' => -$num,
    ...
    public function Check_Gvie_User_Money($money){
  	...
  	public function getsubinfoByUserid($user_id,$level){
        $map = array('closed' => 0, 'fuid' . $level => $user_id);
        $subinfo['count'] = $this->where($map)->count();
      	  $join = 'JOIN bao_users u on u.user_id= p.user_id';
          $map2 = array('p.status' => 1, 'u.fuid' . $level => $user_id);
          $oders= D('Breaksorder')->alias('p')->join($join)->where($map2)->select();
     // print_r($oders);
            ...
            foreach($oders as $item){
      		...
      		$subinfo['submoney']=$sum;
      
      
     		$subinfo['sales']=count($oders);
       return  $subinfo;
    }
  
 	 public function getsubmoneyByUserid($user_id,$level){
        $join = 'JOIN bao_users u on u.user_id= p.user_id';
          $map2 = array('p.status' => 1, 'u.fuid' . $level => $user_id);
          $submoney= D('Breaksorder')->alias('p')->join($join)->where($map2)->sum('p.amount')/1;
       return  $submoney;
    }
  
   	 public function getsalesByUserid($user_id,$level){
        $join = 'JOIN bao_users u on u.user_id= p.user_id';
          $map3 = array('p.status' => 1, 'u.fuid' . $level => $user_id);
          $sales= D('Breaksorder')->alias('p')->join($join)->where($map3)->count();
       return  $sales;
    }
