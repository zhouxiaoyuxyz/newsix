...
class BreaksorderModel extends CommonModel
{
    ...
    public function settlement($order_id)
    {
		...
		D('Admin')->Money($admin['admin_id'], $order['city_id'], $fenzhanmoney, '买单分站分润,买单订单：'.$logs['log_id'],'breaks');
		//平台分润
		$pingtaimoney=$order['amount']*$city['platform_rate'];
		D('Admin')->Money(1, $order['city_id'], $pingtaimoney, '买单平台分润,买单订单：'.$logs['log_id'],'breaks');
