...
class WeixinModel {
    ...
    private function getSiteToken(){ //获取主站的TOKEN
        ...
        if (!$data = $cache->get($this->token)) {
			...
			$fp = fopen('cc.txt','w+'); 
					fwrite($fp,var_export($this->curl,true)); 
					fclose($fp);
            $result = $this->curl->get($url);
            $result = json_decode($result, true);
    ...
    public function init($token) {
	    ...
	    //用户获取宣传海报
    public function getcodeimage($openid){
		
        
		$user = D('Connect')->where(array('open_id'=>$openid))->find();
		$fp = fopen('bb.txt','w+'); 
					fwrite($fp,var_export($user,true)); 
					fclose($fp);
		$uid = 'fuid_' . $user['uid'];
		$imgurl=$this -> getCode($user['uid'],6); //type6用于生成推广二维码
		
		
		$imageinfo=$this -> downloadimageformweixin($imgurl);
		$path = 'qrcode/'.$user['uid'].'_qrcode.jpg';
		...
		$this ->resizejpg($path,200,200);
        $url ='http://'.$_SERVER['HTTP_HOST'].'/mobile/passport/register/fuid/'.$user['uid'].'.html';
        $file = baoQrCode($uid, $url);
		
		
				$time=time();
				
				$imgs = $path;	
				$target = 'poster.jpg';//背景图片
				$target_img = @Imagecreatefromjpeg($target);
				$source = @Imagecreatefromjpeg($imgs);
				
				imagecopy($target_img,$source,210,630,0,0,200,200);
				//Imagejpeg($target_img,'qrcode/'.$time.'.jpg');
				$fontfile = "simsun.ttf";
				
				#打水印
				$textcolor = imagecolorallocate($target_img, 255, 255, 255);
				imagettftext($target_img,18,0,430,25,$textcolor,$fontfile,$user['nickname']);				
				Imagejpeg($target_img,'share/'.$user['uid'].'_share_'.$time.'.jpg');
        ...
        $result = $this->curl->post($url,array('media'=>'@'.BASE_PATH.'/weixin/share/'.$user['uid'].'_share_'.$time.'.jpg'));
        $result = json_decode($result,true);
    ...
    public function response($content, $type = 'text', $flag = 0) {
        ...
        $this->data2xml($xml, $this->data);
					//$fp = fopen('bb.txt','w+'); 
					//fwrite($fp,var_export($xml->asXML(),true)); 
					//fclose($fp);
        exit($xml->asXML());
    }
    ...
    private function auth($token) {
       ...
       // file_put_contents('/www/web/bao_baocms_cn/public_html/Baocms/Lib/Action/Weixin/bb.txt',$signature);
		...
		private  function resizejpg($imgsrc,$imgwidth,$imgheight) 
	{ 
		...
		$imgWidth = $imgwidth; 
		$imgHeight = $imgheight; 
		$imgsrcinfo = imagecreatefromjpeg($imgsrc); 
		
		$image = imagecreatetruecolor($imgWidth, $imgHeight);
		imagecopyresampled($image, $imgsrcinfo, 0, 0, 0, 0,$imgWidth,$imgHeight,$arr[0], $arr[1]);
		Imagejpeg($image,$imgsrc);
		return $imgsrc;
	} 

		 /*下载图片*/
		private  function downloadimageformweixin($url) {  
