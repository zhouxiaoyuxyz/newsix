...
class BreaksorderModel extends CommonModel
{
    ...
    public function settlement($order_id)
    {
		...
		//写入分站资金
		$admin=D('Admin')->where(array('city_id' => $order['city_id']))->find();
		...
		//平台分润
		D('Admin')->Money(1, $order['city_id'], $order['amount']*$city['platform_rate']/100, '买单平台分润,买单订单：'.$logs['log_id'],'breaks');
		D('Admin')->Money($admin['admin_id'], $order['city_id'], $order['amount']-$rate['total']-$money, '买单分站分润,买单订单：'.$logs['log_id'],'breaks');
