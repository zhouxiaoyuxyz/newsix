...
class PassportModel {
    ...
    //UC用邮件登录
    public function login($account, $password, $channelId) {
        $this->token = array(
            'token' => md5(uniqid())
        );
        if ($this->isuc) {
    ...
    public function register($data = array()) {
      ...
      $integral_cash = (int)$this->_CONFIG['integral']['cash'];
		if(!empty($integral_cash)){
			D('Users')->addMoney($data['user_id'],$integral_cash*100,'用户首次注册赠送现金');     	
		}
        setUid($data['user_id']);
        return true;
    }
