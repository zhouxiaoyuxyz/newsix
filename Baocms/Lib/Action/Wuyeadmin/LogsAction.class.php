...
class LogsAction extends CommonAction {
    ...
    public function tixian(){
        ...
        if (IS_POST) {
            ...
            if ($gold <= 0) {
                $this->baoSuccess('提现金额不合法', U('logs/tixian'));
            }
            if ($gold < $cash_money * 100) {
                $this->baoSuccess('提现金额小于最低提现额度', U('logs/tixian'));
            }
            if ($gold > $cash_money_big * 100) {
                $this->baoSuccess('您单笔最多能提现' . $cash_money_big . '元', U('logs/tixian'));
            }
            if ($gold > $admin['gold'] || $admin['gold'] == 0) {
                $this->baoSuccess('商户资金不足，无法提现', U('logs/tixian'));
            }
            if (!($data['bank_name'] = htmlspecialchars($_POST['bank_name']))) {
                $this->baoSuccess('开户行不能为空', U('logs/tixian'));
            }
            if (!($data['bank_num'] = htmlspecialchars($_POST['bank_num']))) {
                $this->baoSuccess('银行账号不能为空', U('logs/tixian'));
            }
            if (!($data['bank_realname'] = htmlspecialchars($_POST['bank_realname']))) {
                $this->baoSuccess('开户姓名不能为空', U('logs/tixian'));
            }
            //获取手机，验证码
            /**$yzm = $this->_post('yzm');
            $s_mobile = session('mobile');
            ...
            $this->assign('money', $admin['gold']/100);
