...
class CommonAction extends Action
{
	...
	{	
		...
		if(!empty($this->_admin['role_id'])){
			if ($this->_admin['role_id'] != 11) {
					session('admin',null);
					$this->error('您不是物业管理员,正在跳转到分站登录页！', U('Wuyeadmin/index/index'));
			}
		}	
	...
	protected function parseTemplate($template = '')
	{
		...
		define('THEME_PATH', BASE_PATH . '/' . APP_NAME . '/Wuyeadmin/');
		define('APP_TMPL_PATH', __ROOT__ . '/' . APP_NAME . '/Wuyeadmin/');
