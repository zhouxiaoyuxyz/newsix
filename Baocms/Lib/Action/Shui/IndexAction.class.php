...
class IndexAction extends CommonAction
{
    public function index()
    {
        ...
        // 计算返现金额，返现到物业费账户 -- by: zhangdianlei
        // $wy_fanxian_money = $shuimachine['wy_fanxian'] * $shuiprice;

        // 计算返现金额，将fx_money分开返给用户 -- by: zhangdianlei
        static $CONFIG;
        if (empty($CONFIG)) {
            $CONFIG = D('Setting')->fetchAll();
        }
        $wy_fanxian_money = $CONFIG["site"]["cut"] * $fx_money;
        $fx_money = $fx_money - $wy_fanxian_money;
        ...
        if($user['money']>=$shuiprice){
            ...
            //判断是否是年卡，如果是不添加物业费，0不是，1是
            if($user['setshenka']==0){
                D('Users')->AddWyFanxian($user['user_id'], $wy_fanxian_money * 100, '惠水源购水，物业费返现');
            }
            ...
            // zhangdianlei add 2019-03-17
            //判断是否是年卡，如果是不添加助力互助金，0不是，1是
            if($user['setshenka']==0){
                if($shuiprice > 0){
