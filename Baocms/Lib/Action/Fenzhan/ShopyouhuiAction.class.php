...
class ShopyouhuiAction extends CommonAction{
    ...
    public function index(){
	 ...
	 public function edit($yh_id = 0) {
		...
		$rate['breaks_profit_rate']=($this->_CONFIG['profit']['breaks_profit_rate1']+$this->_CONFIG['profit']['breaks_profit_rate2']+$this->_CONFIG['profit']['breaks_profit_rate3'])*100;
		$rate['platform_rate']=$this->citys[0]['platform_rate'];
    ...
    private function createCheck() {
        ...
        if($data['type_id'] == 0){ //折扣优惠
            ...
            if (!empty($data['deduction'])) {
				...
				}elseif($data['deduction'] > (100-$data['discount'])){
					$this->baoError('扣除点数不能大于除商家外剩余金额！');
				}elseif($data['deduction'] < $rate['total']){
					$this->baoError('扣除点数不能小于正常运营费用'.$rate['total'].'%');
				}
            }
