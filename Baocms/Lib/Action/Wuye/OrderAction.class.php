...
class OrderAction extends CommonAction {
    ...
    public function create($owner_id) {
        ...
        if (!$detail = D('Communityowner')->where(array('owner_id' => $owner_id, 'community_id' => $this->community_id))->find()) {
            $this->error('该业主不存在');
        }
        ...
        if ($this->isPost()) {
            ...
            if (!$res = $obj->where(array('owner_id' => $owner_id, 'order_date' => $data['order_date']))->find()) {
                if ($order_id = $obj->add($data)) {
                    ...
                    $this->fengmiMsg('添加成功', U('order/index', array('owner_id' => $owner_id)));
                } else {
                    $this->fengmiMsg('添加失败');
                }
            }
            $this->fengmiMsg('该账单已存在',U('order/index', array('owner_id' => $owner_id)));
        } else {
