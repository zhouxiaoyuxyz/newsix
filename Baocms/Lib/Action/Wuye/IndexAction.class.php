...
class IndexAction extends CommonAction {
	...
	$counts['feedback_day'] = (int) D('Feedback')->where(array(
	...
	$counts['news_day'] = (int) D('Communitynews')->where(array(
		'create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time)),
		'community_id' => $this->community_id,
		'closed'=>0
	))->count();
	...
	$counts['owner_day'] = (int) D('Communityowner')->where(array(
		'create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time)),
		'community_id' => $this->community_id,
		'closed'=>0
	))->count();
...
$counts['order_type_5_is_pay'] = (int) D('Communityorderproducts')->where(array('type'=>5,'community_id' => $this->community_id,'is_pay'=>0))->sum('money');
	...
	//财务管理
	$counts['order'] = (int) D('Communityorderproducts')->where(array('type'=>5,'community_id' => $this->community_id))->sum('money');
	
	$counts['order_1'] = (int) D('Communityorderproducts')->where(array('type'=>5,'community_id' => $this->community_id))->sum('paid');//已交费
	$counts['order_0'] = $counts['order']-$counts['order_1'];//未交费
	//论坛
	...
	$counts['bbs_day'] = (int) D('Communityposts')->where(array(
		'create_time' => array(array('ELT', NOW_TIME), array('EGT', $bg_time)),
		'community_id' => $this->community_id,
		'closed'=>0
	))->count();
