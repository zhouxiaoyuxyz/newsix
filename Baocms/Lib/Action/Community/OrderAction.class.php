...
class OrderAction extends CommonAction {
    ...
    public function index() { //业主账单列表
        ...
        if ($owner_id = (int) $this->_param('owner_id')) {
		...
		$owner=D('Communityowner')->itemsByIds($owner_ids);
		//print_r($owner);
    ...
    public function create($owner_id) {
        ...
        if (!$detail = D('Communityowner')->where(array('owner_id' => $owner_id))->find()) {
            $this->error('该业主不存在');
        }
        ...
        if ($this->isPost()) {
            ...
            if (!$res = $obj->where(array('owner_id' => $owner_id, 'order_date' => $data['order_date']))->find()) {
                if ($order_id = $obj->add($data)) {
                    ...
                    $this->baoSuccess('添加成功', U('order/index', array('owner_id' => $owner_id)));
                } else {
                    $this->baoError('添加失败');
                }
            }
    ...
    public function edpay($owner_id, $pay_id) {
	 ...
	 public function is_pay($id=0) {
        ...
        $total=1;
		$user_id = $Communityorder['owner_id'];
		$reduction = $detail['money'] - $detail['paid'];
		$obj->save(array('id' => $id, 'paid' => $detail['money'], 'is_pay' => 2));
