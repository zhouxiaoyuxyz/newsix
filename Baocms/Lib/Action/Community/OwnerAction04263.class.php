    ...
    public function importok(){
        ...
        foreach($owners as $val){
            if(!empty($val)){				//print_r($val);				foreach($val as $key =>$val1){
				...
				$isowner = D('Communityowner')->where(array('dong' => $data['dong'],'danyuan' => $data['danyuan'],'number' => $data['number'],'community_id' => $this->community_id))->find();
				print_r($isowner);
				if(empty($isowner)){
					//print_r($data);
					$owner_id = $obj->add($data);
					//print_r($owner_id);
					if (!$res = D('Communityorder')->where(array('owner_id' => $owner_id, 'order_date' => $data['order_date']))->find()) {
						$data[owner_id]=$owner_id;
						if ($order_id = D('Communityorder')->add($data)) {
					...
					if (!$res = D('Communityorder')->where(array('owner_id' => $owner_id, 'order_date' => $data['order_date']))->find()) {
						$data[owner_id]=$isowner[owner_id];
						if ($order_id = D('Communityorder')->add($data)) {
							D('Communityorderproducts')->add(array(
										 'order_id' => $order_id, 
										 'community_id' => $this->community_id,
