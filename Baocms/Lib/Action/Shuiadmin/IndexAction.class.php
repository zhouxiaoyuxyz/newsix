...
class IndexAction extends CommonAction
{
    ...
    public function main(){
		...
		$join = 'JOIN bao_community c on c.community_id= p.community_id';
		$join2 = 'JOIN bao_community c on c.community_id= o.community_id';
        $counts['users'] = (int) D('Users')->where(array('closed' => 0))->count();
        $counts['communities'] = (int) D('Community')->where(array('closed' => 0, 'uid' => $admin_id))->count();
        $counts['wuyemoney'] = (int) D('Communityorderproducts')->alias('p')->join($join)->where(array('c.uid' => $admin_id))->sum('p.money')/1;
        $counts['reduction'] = (int) D('Communityorderlogs')->alias('o')->join($join2)->where(array('c.uid' => $admin_id))->sum('o.reduction')/1;
        $counts['wuyerealpaid'] = (int) D('Communityorderlogs')->alias('o')->join($join2)->where(array('c.uid' => $admin_id))->sum('o.money')/1;
        $counts['wuyepaid'] = (int) D('Communityorderproducts')->alias('p')->join($join)->where(array('c.uid' => $admin_id))->sum('p.paid')/1;
        $counts['wuyeneedpay'] = (int) $counts['wuyemoney']-$counts['wuyepaid'];
