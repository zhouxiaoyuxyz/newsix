...
class WeixinAction extends CommonAction {
	public function _initialize() {
		...
		if($this->workers['tuan'] != 1 && empty($shop)){
    ...
    public function tuan() {
		...
		if(($data['shop_id'] != $worker['shop_id'] || $worker['tuan']!=1) && $user_id!= $shop['user_id']){
		...
		if ((int) $data['is_used'] == 0 && (int) $data['status'] == 0) {
			...
			if ($obj->save(array('code_id' => $data['code_id'], 'is_used' => 1,'used_time' => NOW_TIME,'worker_id' => $this->uid, 'used_ip' => $ip))) { //这次更新保证了更新的结果集
				...
				$fanmoney = (int) ($order['mobile_fan'] / $order['num']);

				// 返现到物业费 -- by: zhangdianlei
                static $CONFIG;
                if (empty($CONFIG)) {
                    $CONFIG = D('Setting')->fetchAll();
                }
                $wy_fanxian_money = $CONFIG["site"]["cut"] * $fanmoney;
                $fanmoney = $fanmoney - $wy_fanxian_money;
                D('Users')->AddWyFanxian($data['user_id'], $wy_fanxian_money, '享乐购返物业费：订单ID' . $order['order_id']);
                  
                // 抢购助力互助金 -- by: zhangdianlei 2019-03-17
                // 抢购满5元，返0.1元互助金
                if($data['settlement_price'] >= 500){
				...
				//抢购返现
				D('Users')->addMoney($data['user_id'], $fanmoney, '抢购返现：订单ID' . $order['order_id']);
			...
			$this->error('该团购券无效或已经使用！',U('/store/index/index'));
		}
    }
