    ...
    public function tuan() {
		...
		if ((int) $data['is_used'] == 0 && (int) $data['status'] == 0) {
			...
			if ($obj->save(array('code_id' => $data['code_id'], 'is_used' => 1,'used_time' => NOW_TIME,'worker_id' => $this->uid, 'used_ip' => $ip))) { //这次更新保证了更新的结果集
				...
				$fanmoney = (int) ($order['mobile_fan'] / 100 / $order['num']);
                ...
                D('Users')->AddWyFanxian($data['user_id'], $wy_fanxian_money * 100, '享乐购返物业费：订单ID' . $order['order_id']);


				D('Users')->addMoney($data['user_id'], $fanmoney*100, '抢购返现：订单ID' . $order['order_id']);
