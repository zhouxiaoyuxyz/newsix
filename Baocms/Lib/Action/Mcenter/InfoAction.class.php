...
class InfoAction extends CommonAction{
	...
	public function shuicar()
    {
        if ($this->isPost()) {
            $shuicar1 = $this->_post('shuicar1', 'htmlspecialchars');
           
            $shuicar2 = $this->_post('shuicar2', 'htmlspecialchars');
            if (empty($shuicar1) && empty($shuicar2)) {
                $this->fengmiMsg('请至少绑定一张卡！');
            }
            $map['shuicar1|shuicar2'] = $shuicar1;
     		$user=D('Users')->where($map)->find();
            if(!empty($user)){
            	$this->fengmiMsg('该卡已被绑定！');
            }
            if (D('Users')->save(array('user_id' => $this->uid, 'shuicar1' => $shuicar1, 'shuicar2' => $shuicar2))) {
              
                // 小北绑卡
                if ($shuicar1 != "") {
                    $shui_map1['shui_card_no'] = $shuicar1;
                    $shui_classify = D('Shuiclassifycards')->where($shui_map1)->find();
                    $card_name = $shui_classify['classify_no'];
                    $map_card1['shuicar1'] = $shuicar1;
                    $user = D('Users')->where($map_card1)->find();

                    $url_string = 'http://211.149.129.226:8080/XiaoBeiMgtsys/water/add?card_number='. $shuicar1 .'&card_name='. $card_name  .'&name='. $user['nickname'] .'&tel='. $user['mobile'] .'&id_number=&now_money=0&user_id=' . $user['user_id'];
                    //echo $url_string;
                    //$this->fengmiMsg($url_string);
                ...
                if ($shuicar2 != "") {
                    $shui_map2['shui_card_no'] = $shuicar2;
                    $shui_classify = D('Shuiclassifycards')->where($shui_map2)->find();
                    $card_name = $shui_classify['classify_no'];
                    $map_card2['shuicar2'] = $shuicar2;
                    $user = D('Users')->where($map_card2)->find();

                    $url_string = 'http://211.149.129.226:8080/XiaoBeiMgtsys/water/add?card_number='. $shuicar2 .'&card_name='. $card_name  .'&name='. $user['nickname'] .'&tel='. $user['mobile'] .'&id_number=&now_money='. 0 .'&user_id=' . $user['user_id'];
                ...
                $this->fengmiMsg('绑卡成功！', U('index/index'));
