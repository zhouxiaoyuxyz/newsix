...
class LogsAction extends CommonAction{
    ...
    /**
     * zhangdianlei 2018.12.11
     */
    public function wy_fanxianlogs(){
        $this->display();
    }
     ...
     * 物业返现log记录数据
     */
    public function wy_fanxian_data(){
        $Userwyfanxianlogs = D('Userwyfanxianlogs');
        import('ORG.Util.Page');
        $map = array('user_id' => $this->uid);
        $count = $Userwyfanxianlogs->where($map)->count();
        $Page = new Page($count, 10);
        $show = $Page->show();
        ...
        $list = $Userwyfanxianlogs->where($map)->order(array('log_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $this->assign('list', $list);
        $this->assign('page', $show);
        $this->display();
    }
