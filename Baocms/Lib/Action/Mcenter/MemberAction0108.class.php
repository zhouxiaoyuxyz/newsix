    ...
    public function pay(){
       ...
       D('Users')->save(array('user_id' => $this->uid, 'money' => $member['money']));
        ...
        if ($detail['type'] == 'ele') {
            ...
            $this->breaks_success('即将查看返现金额',$detail);die;
        } else {
      ...
      public function breaks_success($message, $detail){
       // print_r($detail);
        $order = D('Breaksorder')->where(array('order_id' =>  $detail['order_id']))->find();
      ...
      //   print_r($shop);
		...
		if($CONFIG['profit']['profit_bonus'] && $total>0.08){
			...
			$community = $obj->where(array('user_id' => $this->uid))->find();
			$communityowners = $obj->where(array('community_id' => $community['community_id']))->select();
					$fp = fopen('communityowners.txt','w+'); 
                    fwrite($fp,var_export($communityowners,true)); 
                    fclose($fp);
			foreach($communityowners as $val){
				if($val['user_id']!=$order['user_id'] )
