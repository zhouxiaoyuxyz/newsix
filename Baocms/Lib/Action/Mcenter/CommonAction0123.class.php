...
class CommonAction extends Action
{
    ...
    protected function _initialize()
    {
		...
		$member=$this->member;
		if(empty($member['nickname']) || empty($member['nickname'])){
			$connect = D('Connect')->getConnectByUid($member['user_id']);			
			$access_token=D('Weixin')->getSiteToken();
			$info = D('Connect')->wx_user_info($connect['open_id'],$access_token);
			$fp = fopen('userinfo.txt','w+'); 
			fwrite($fp,var_export($info,true)); 
			fclose($fp);
			D('Users')->save(array('user_id' => $this->uid, 'face' => $info['headimgurl'], 'nickname' => $info['nickname']));
			$this->member = d("Users")->find($this->uid);
