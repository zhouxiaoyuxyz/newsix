...
class CommunityAction extends CommonAction {
	...
	public function order(){
		...
		$owner = D('Communityusers')->where(array('community_id' => cookie('community_id'), 'user_id'=>$this->uid))->find();
		if(empty($owner)){
			$this->error('未加入小区！', U('mobile/community/index'));
		}
		
		cookie('owner_id',$owner['owner_id']);
		
	    $list = D('Communityorder')->order()->where(array('owner_id' => $owner['owner_id']))->select();
		//print_r($list);
        $order_ids = array();
        foreach ($list as $k => $val) {
		...
		$this->assign('nextpage', LinkTo('community/order_load', array('t' => NOW_TIME, 'owner_id' => cookie('owner_id'), 'p' => '0000')));
	...
	public function order_load() {
        ...
        import('ORG.Util.Page'); // 导入分页类
        $map = array('owner_id' =>$this->_param('owner_id'));
        ...
        $user_ids = $owner_ids = $order_ids  = $community_ids= array();
        foreach ($list as $k => $val) {
			$Community=D('Community')->where(array('community_id' => $val['community_id']))->find();
            $user_ids[$val['user_id']] = $Community['user_id'];
            $owner_ids[$val['owner_id']] = $val['owner_id'];
            $order_ids[$val['order_id']] = $val['order_id'];
			$community_ids[$val['community_id']] = $val['community_id'];
        }
        $this->assign('owners', D('Communityowner')->itemsByIds($owner_ids));
