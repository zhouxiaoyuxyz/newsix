...
class OrderAction extends CommonAction {
   public function _initialize() {
    ...
    public function distribution() {
        ...
        }else if(($order['status'] != 2) && ($order['status']!=1)){
	...
	//一键发货
	public function oneexpress($order_id = 0){
        ...
        if ($detail['closed'] != 0) {
			...
			D('Order')->where(array('order_id' => $order_id, 'closed' => 0))->setField('status', 2);//更改配送状态
			//D('Order')->order_delivery($order_id);//执行发货
			//$this->ajaxReturn(array('status' => 'success', 'msg' => '一键发货成功', 'url' => U('order/wait')));
			$this->baoSuccess('一键发货成功！', U('order/wait'));
