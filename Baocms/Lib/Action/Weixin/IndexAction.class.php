            ...
            case 'event':
				...
				if ($data['Event'] == 'CLICK') {
					...
					if ($data['EventKey'] == 'qrcode') {
						//$this->weixin->response("正在生成专属海报，请稍后。。。", 'text');
						//$content['MediaId']="qI6_Ze_6PtV7svjolgs-rN6stStuHIjs9_DidOHaj0Q-mwvBelOXCFZiq2OsIU-p";
						//$this->weixin->response($content, 'image');
						$content['MediaId']=$this->weixin->getcodeimage($data['FromUserName']);
						$this->weixin->response($content, 'image');
					}
                }
                break;
...
';
                ...
                if (empty($ding) && count($list) == 0) {
					if ($data['Content']=='qrcode'){
						 $content['MediaId']=$this->weixin->getcodeimage($data['FromUserName']);
						$this->weixin->response($content, 'image');
						 die;
					}else{
						$text = '亲爱的朋友你的反馈我们已经收到，我们会安排专职人员与您沟通、汇报、交流……如果你着急的话下列问题可以回复“数字”得到精准答复哟……
1-如何查看余额
2-如何免费饮用山泉水
3-商家&合伙人提现问题
4-如何绑卡
...
10-客服电话';
    ...
    private function events()
    {
        ...
        if (!empty($data['data'])) {
            ...
            if ($type == 1) {
				...
				$info_url = 'https://api.weixin.qq.com/cgi-bin/user/info?access_token=' . $token . '&openid=' . $data['data']['FromUserName'] . '&lang=zh_CN';
				$info = $curl->get($info_url);
				$info = json_decode($info, true);
				$content[] = array('欢迎光临'.$shop['shop_name'].'【点此付款】', $shop['addr'], $this->getImage($shop['photo']), __HOST__ . '/mobile/shop/breaks/shop_id/' . $shop_id . '.html');
				$data = array('type' => 'weixin', 'open_id' => $info['openid'], 'nickname' => $info['nickname'], 'headimgurl' => $info['headimgurl']);
				
				$jieguo=$this->wxregister($data,$shop['user_id']);
				 if ($jieguo==1){
					 static $CONFIG;
					if (empty($CONFIG)) {
						$CONFIG = D('Setting')->fetchAll();
					}
					D('Users')->Money($shop['user_id'], $CONFIG['profit']['profit_shangjia']*100, '商家新用户推荐奖励');
					 $this->weixin->response($content, 'news');
				...
				$jieguo=$this->wxregister($data,$fuid1);
				 if ($jieguo==1){
					$this->weixin->response("恭喜您注册成功", 'text');
				} else {
					$this->weixin->response("您已经注册过本站会员", 'text');
				}
    ...
    {
      $fp = fopen('dd.txt','w+'); 
					fwrite($fp,var_export(__HOST__,true));
					fclose($fp);
        return __HOST__ . '/attachs/' . $img;
    }
	
	//微信自动注册为用户
    private function wxregister($data,$fuid1)
    {
        $connect = D('Connect')->getConnectByOpenid($data['type'], $data['open_id']);
		//$fp = fopen('ee.txt','w+'); 
					//fwrite($fp,var_export($connect,true)); 
					//fclose($fp);
        if (empty($connect)) {
            $connect = $data;
        ...
        if (empty($connect['uid'])) {
            ...
            $user = array('account' => $account, 'password' => rand(10000000, 999999999), 'nickname' => $nickname, 'ext0' => $account, 'face' => $data['headimgurl'], 'token' => $data['token'], 'reg_time' => NOW_TIME, 'reg_ip' => get_client_ip(), 'fuid1' => $fuid1);
