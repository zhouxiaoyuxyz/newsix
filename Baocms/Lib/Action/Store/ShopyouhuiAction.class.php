...
class ShopyouhuiAction extends CommonAction {
    ...
    public function index() {
        if ($this->isPost()) {
            ...
            if(!$data['yh_id']){
                if ($obj->add($data)) {
                    $this->fengmiMsg('添加成功', U('shopyouhui/index'));
                ...
                if (false !== $obj->save($data)) {
                   $this->fengmiMsg('修改成功', U('shopyouhui/index'));
                }
                $this->fengmiMsg('操作失败！');
            }
        }else{
    ...
    private function createCheck() {
        ...
        if($data['type_id'] == 0){ //折扣优惠
            ...
            if (empty($data['discount'])) {
                $this->fengmiMsg('折扣不能为空');
            }elseif(!is_numeric($data['discount'])){
                $this->fengmiMsg('折扣格式不正确');
            }
        }else{
            $data['min_amount'] = $data['min_amount'];
            if (empty($data['min_amount'])) {
                $this->fengmiMsg('满多少不能为空');
            }elseif(!is_numeric($data['discount'])){
                $this->fengmiMsg('满多少格式不正确');
            }
            $data['amount'] = $data['amount'];
            if (empty($data['amount'])) {
                $this->fengmiMsg('减多少不能为空');
            }elseif(!is_numeric($data['discount'])){
                $this->fengmiMsg('减多少格式不正确');
            }
        }
		$data['audit'] = 0;
    ...
    public function close(){
        if(false !== D('Shopyouhui')->data(array('is_open'=>0))->where(array('shop_id'=>$this->shop_id))->save()){
            $this->fengmiMsg('成功关闭优惠买单',U('shopyouhui/index'));
        }
    }
    
    public function open(){
        if(false !== D('Shopyouhui')->data(array('is_open'=>1))->where(array('shop_id'=>$this->shop_id))->save()){
            $this->fengmiMsg('成功开启优惠买单',U('shopyouhui/index'));
        }
    }
