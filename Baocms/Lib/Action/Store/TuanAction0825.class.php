...
class TuanAction extends CommonAction
{
    ...
    }			public function xiaofei() {        if ($this->isPost()) {		print_r($_POST);           if (isMobile(htmlspecialchars($_POST['account']))) {                if (!$scode = trim($_POST['scode'])) {                    $this->fengmiMsg('请输入短信验证码！');                }                $scode2 = session('scode');                if (empty($scode2)) {                    $this->fengmiMsg('请获取短信验证码！');                }                if ($scode != $scode2) {                    $this->fengmiMsg('请输入正确的短信验证码！');                }            }			$ip = get_client_ip();            $shopmoney = D('Shopmoney');							$shopmoney->add(array(                                    'shop_id' => trim($_POST['shop_id']),                                    'money' => $_POST['fee']*100,                                    'create_ip' => $ip,                                    'create_time' => NOW_TIME,                                    'order_id' => 6,                                    'intro' => "线下消费",                                ));                                $shop = D('Shop')->find($_POST['shop_id']);                                D('Users')->addMoney($shop['user_id'], $_POST['fee']*100, '线下消费');								$user = D('Users')->getUserByMobile($_POST['account']);                                D('Users')->addMoney($user['user_id'], -$_POST['fee']*100, '线下消费');					$randstring = rand_string(6, 1);					session('scode', $randstring);                  $this->fengmiMsg('消费成功！',U('tuan/xiaofei'));				                     } else {            $this->display();        }    }		public function xfyz() {        if (!$mobile = htmlspecialchars($_POST['mobile'])) {            die('请输入正确的手机号码！');        }        if (!isMobile($mobile)) {            die('请输入正确的手机号码！');        }		if ($_POST['fee']<=0) {            die('请输入正确消费金额！');        }		$user = D('Users')->getUserByMobile($mobile);		//print_r($user);		if (empty($user)) {            die('该用户不存在！');        }		$shops = D('Shop')->find($_POST['shop_id']);		$isshop = D('Shop')->getShopIdsByUserId($user['user_id']);		if (!empty($isshop)) {            die('商户账户不能用于消费！请重新注册！');        }        if ($user['money']/100 < $_POST['fee']) {            die('账户余额不足！');        }		$user2 = D('Users')->getUserByUserid($shops['user_id']);		//print_r($user2);		        $randstring = session('scode');        if (empty($randstring)) {            $randstring = rand_string(6, 1);            session('scode', $randstring);        }        //        D('Sms')->sendSms('sms_xf', $mobile, array('code' => $randstring,'nickname' => $user['nickname'],'shopname' => $shops['shop_name'],'fee' => $_POST['fee']));        die('1');    }
}