...
class UsercashAction extends CommonAction{
    ...
    public function audit_wuye($cash_id = 0, $status = 0){
        ...
        if ($cash_id = (int) $cash_id) {
            ...
            if ($data['status'] == 0) {
                ...
                $arr['status'] = $status;
                $Userscash->save($arr);
                $this->baoSuccess('操作成功！', U('usercash/wuye'));
            } else {
                $this->baoError('操作失败');
            }
        }
    ...
    public function audit_fenzhan($cash_id = 0, $status = 0){
        ...
        if ($cash_id = (int) $cash_id) {
            ...
            if ($data['status'] == 0) {
                ...
                $arr['status'] = $status;
                $Userscash->save($arr);
                $this->baoSuccess('操作成功！', U('usercash/fenzhan'));
            } else {
                $this->baoError('操作失败');
            }
        }
    ...
    public function jujue(){
        ...
        if ($status == 2) {
            ...
            D('Weixintmpl')->weixin_cash_user($detail['user_id'],2);//申请提现：1会员申请，2商家同意，3商家拒绝
    ...
    public function jujue_wuye(){
        ...
        $money = $detail['money'];
        if ($status == 2) {
            D('Admin')->Money($detail['admin_id'], 0, $money, '提现拒绝，退款');
            D('Userscash')->save(array('cash_id' => $cash_id, 'status' => $status, 'reason' => $value));
