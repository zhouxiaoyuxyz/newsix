...
class CityAction extends CommonAction {
    ...
    private $create_fields = array('name', 'pinyin','photo', 'is_open','platform_rate', 'lng', 'lat','orderby','theme','first_letter','domain');
    private $edit_fields = array('name', 'pinyin','photo', 'is_open', 'platform_rate','lng', 'lat','orderby','theme','first_letter','domain');
    ...
    public function delete($city_id = 0) {
        if (is_numeric($city_id) && ($city_id = (int) $city_id)) {
			...
			$obj->delete($city_id);
            $obj->cleanCache();
            //$obj->save(array('city_id' => $city_id, 'closed' => 1));
            ...
            if (is_array($city_id)) {
                ...
                foreach ($city_id as $id) {
					$obj->delete($id);
                    // $obj->save(array('city_id' => $id, 'closed' => 1));
                }
                $this->baoSuccess('删除成功！', U('city/index'));
            }
