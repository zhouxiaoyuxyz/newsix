        ...
        if (!empty($order_id)) {
            ...
            if ($goods['status'] == 1) {
                ...
                if ($goods['is_daofu'] == 0) {
                    ...
                    $money = $goods['js_price'];
                  /**  if ($goods['tui_uid']) { //推广员分成
                        $gooddetail = D('Goods')->find($goods['goods_id']);
                        if (!empty($gooddetail['commission']) && $gooddetail['commission'] < $gooddetail['mall_price'] && $gooddetail['commission'] < $goods['total_price']) { //小于的情况下才能返利不然你懂的
                            $money -= $gooddetail['commission'];
