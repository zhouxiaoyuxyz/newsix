...
class UserAction extends CommonAction {
    ...
    public function index() {
		...
		if($shuicar = $this->_param('shuicar','htmlspecialchars')){
            $map['shuicar1|shuicar2'] = array('LIKE','%'.$shuicar.'%');
            $this->assign('shuicar',$shuicar);
        }
        ...
        foreach($list as $k=>$val){
          ...
          $subinfo=D('Users')->getsubinfoByUserid($val['user_id'],1);
          $val['subordinate']=$subinfo['count'];
          $val['submoney']=  $subinfo['submoney'];
          //$map = array('closed' => 0, 'fuid1' =>  $val['user_id']);
          //$val['subordinate']= D('Users')->where($map)->count();
         // $join = 'JOIN bao_users u on u.user_id= p.user_id';
         // $map2 = array('p.status' => 1, 'u.fuid1' =>  $val['user_id']);
         // $val['submoney']= D('Breaksorder')->alias('p')->join($join)->where($map2)->sum('p.amount')/1;
            $list[$k] = $val;
        }
