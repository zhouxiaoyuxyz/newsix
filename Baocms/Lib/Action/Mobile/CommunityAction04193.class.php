...
class CommunityAction extends CommonAction{
    ...
    public function owner($community_id){
        ...
        if ($this->isPost()) {
			...
			$where['dong'] = $_POST['dong'];
			$where['danyuan'] = $_POST['danyuan'];
            ...
            if (!($res = $obj->where(array('user_id' => $this->uid))->find())) {
                if ($obj->add($data)) {										
					$bonusId =  D('Bonus')->create_bonus(2,8,0.01);
                    $this->fengmiMsg('申请成功，请等待物业审核', U('/mobile/bonus/index/bonus_id/'.$bonusId));
                }
                $this->fengmiMsg('申请失败');
            }
