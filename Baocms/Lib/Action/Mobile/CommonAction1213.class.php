...
class CommonAction extends Action{
    ...
    protected function life_yzm()
    {
        ...
        //这里换一个名字code，要求10分钟内有效，或者2分钟，最好可以设置。
    ...
    protected function mobile2()
    {
        ...
        if ($this->isPost()) {
            ...
            $data = array('user_id' => $this->uid, 'ext_mobile' => $mobile);
            if (D('Users')->save($data)) {
                $this->ajaxReturn(array('status' => 'success', 'msg' => '恭喜您成功绑定副卡！'));
            }
            $this->ajaxReturn(array('status' => 'error', 'msg' => '更新失败！'));
        }
    }
    ...
    protected function fengmiMsg($message, $jumpUrl = '', $time = 3000)
    {
        ...
        $str .= 'window.location.href="https://"+window.location.host+"' . $jumpUrl . '";';
        $str .= '</script>';
        die($str);
    }
