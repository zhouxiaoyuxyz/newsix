...
class PaymentAction extends CommonAction {
    ...
    public function respond() {
        ...
        if (!empty($detail)) {
            if ($detail['type'] == 'ele') {
                ...
                $this -> success('物业缴费成功！', U('mcenter/community/order'));
            }
    ...
    public function yes($log_id) {
        ...
        switch ($logs['type']) {
            ...
            case 'community' :
                ...
                break;
            default :
                $this -> success('恭喜您支付成功啦!,正在跳转查看返现记录...', U('mcenter/logs/wy_fanxianlogs'));
                break;
        }
    }
    ...
    public function payment($log_id) {
        ...
        if($logs['type']=='tuan'){
            ...
            $fanxian= $tuanorder['mobile_fan']*0.8;
            $this -> assign('fanxian', $fanxian);
        }else if($logs['type']=='breaks'){
            $breaksorder = D('Breaksorder') -> find($logs['order_id']);
            $fanxian= $breaksorder['fx_money']*100;
            $this -> assign('fanxian', $fanxian);
        }else if($logs['type']=='activity'){
            $activityorder = D('Activityorder') -> find($logs['order_id']);
            $activity=D('Activity') -> find($activityorder['activity_id']);
            $fanxian= $activity['shui_fanxian']+$activity['wuye_fanxian'];
            $this -> assign('fanxian', $fanxian);
        }
