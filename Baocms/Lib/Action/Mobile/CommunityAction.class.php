...
class CommunityAction extends CommonAction{
    ...
    public function owner($community_id){
		...
		$dong_list = $obj->Distinct(true)->where(array('community_id' => $community_id))->getField('dong',true);
		//print_r($dong_list);
		
        if ($this->isPost()) {
			
			if (empty($_POST['owner_id'])) {
                $this->fengmiMsg('业主不存在！');
            }
			...
			if (!($res = $obj->where(array('user_id' => $this->uid))->find())) {
				static $CONFIG;
				if (empty($CONFIG)) {
				...
				if ($obj->add($data)) {
					if($CONFIG['profit']['profit_bonus']){
						$bonusId=D('Bonus')->create_bonus($CONFIG['profit']['bonus_profit_amount'],$CONFIG['profit']['bonus_profit_num'],0.01);
						 $this->fengmiMsg('欢迎您加入' . $detail['name'] . '小区！请领取红包！', U('/mobile/bonus/index/bonus_id/'.$bonusId));
					}
            ...
            $this->fengmiMsg('不能重复申请', U('community/detail', array('community_id' => $community_id)));
           
              
            
        } else {
            $this->assign('detail', $detail);
            $this->assign('dong_list', $dong_list);
            $this->display();
        }
    }
    ...
    //获取单元号
    public function get_danyuan(){
        $listObj =  D('Communityowner');
        $where['dong'] = (int) $this->_get('dong');
		$where['community_id'] = cookie('community_id');
		//print_r($where);
        $list = $listObj->Distinct(true)->where($where)->getField('danyuan',true);
        $data=array('status'=>0,'danyuan'=>$list);
    ...
    public function get_number(){
        $listObj =  D('Communityowner');
		$where['dong'] = (int) $this->_get('dong');
        $where['danyuan'] = (int) $this->_get('danyuan');
		$where['community_id'] = cookie('community_id');
        $list = $listObj->Distinct(true)->where($where)->getField('number',true);
        $data=array('status'=>0,'number'=>$list);
        header("Content-type: application/json");
        exit(json_encode($data));
    }
    ...
    public function get_owner(){
		...
		$where['dong'] = (int) $this->_get('dong');
        $where['danyuan'] = (int) $this->_get('danyuan');
        $where['number'] = (int) $this->_get('number');
		$where['community_id'] = cookie('community_id');
        $owner = $listObj->where($where)->find();
		$owner['phone'] = substr_replace($owner['phone'],'****',3,4);
		$owner['name'] = mb_substr($owner['name'],0,1,'UTF8').'*';
		//print_r($owner);
        $data=array('status'=>0,'owner'=>$owner);
        header("Content-type: application/json");
        exit(json_encode($data));
    }
  ...
  //小区主页
    ...
    public function detail($community_id){
        ...
        $this->assign('ads', $ads);
        $this->assign('counts', $counts = D('Communityusers')->where(array('community_id' => $community_id))->count());// 统计邻居人数
        $this->assign('user_owner', $user_owner = D('Communityowner')->where(array('community_id' => $community_id))->count());// 统计业主人数
        //账单
		$owner=D('Communityusers')->where(array('community_id' => $community_id, user_id => $this->uid))->find();
        $zhangdan = D('Communityorder')->order(array('order_date' => 'desc'))->where(array('owner_id' => $owner['owner_id']))->select();
        $owner_ids = $order_ids = array();
        foreach ($zhangdan as $k => $val) {
            $owner_ids[$val['owner_id']] = $val['owner_id'];
  	...
  	public function detail_aibox($community_id){
        $community_id = (int) $community_id;
        $community = D('Community');
        if (empty($community_id)) {
            $this->error('没有该小区');
    ...
    public function post(){
        ...
        if ($this->isPost()) {
            ...
            $data['audit'] = $this->_CONFIG['site']['xiaoqu_post_audit'];
            //回帖是否免审核
    ...
    private function postCheck()
    {
        ...
        $data['gallery'] = htmlspecialchars($data['photo']);
    ...
    public function out(){
        if (empty($this->uid)) {
            $this->error('您还未登陆！', U('passport/login'));
        }
        $community_id = (int) $this->_get('community_id');
        if (!($detail = D('Community')->find($community_id))) {
            $this->error('没有该小区');
    ...
    public function order($community_id){
        ...
        if (!($res = D('Communityowner')->where(array('community_id' => $community_id, 'owner_id' => cookie('owner_id')))->find())) {
            redirect(U('community/owner', array('community_id' => $community_id)));
        }
        $obj = D('Communityorder');
        $map = array('owner_id' => cookie('owner_id'), 'community_id' => $community_id);
    ...
    public function orderpay()
    {
        ...
        if (IS_AJAX) {
            ...
            $detail = D('Communityorder')->where(array('community_id' => $community_id, 'order_date' => $order_date, 'owner_id' => cookie('owner_id')))->find();
            if (empty($detail)) {
            ...
            //if ($res === $type) {
                ...
                if ($total2 == $total) {
                    if ($this->member['money'] < $total * 100) {
                        ...
                        $this->ajaxReturn(array('status' => 'error', 'msg' => '账户余额不足,跳转微信支付', 'url' => '/mobile/community/orderpaybyweixin/log_id/'.$logs['log_id']));
                    } else {
	...
	//微信缴物业费
    public function orderpaybyweixin()
    {
