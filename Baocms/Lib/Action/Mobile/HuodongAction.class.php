...
class HuodongAction extends CommonAction {
    ...
    public function index() {
      ...
      public function paiming() {
        ...
        $join = 'JOIN bao_users u on u.user_id= p.user_id';
         $list = $Activitysign->alias('p')->join($join)->where($map)->order(array( $type=>'desc'))->limit(0,10)->select();
        //print_r($list);
    ...
    public function detail() {      
        ...
        if (!empty($sign)) {
          ...
          // print_r($sign);
      		$subinfo=D('Users')->getsubinfoByUserid($this->uid,1);
     		// print_r($subinfo);
       		 $data['num'] =$subinfo['count']-$sign['original_num'];
 			 $data['submoney'] =$subinfo['submoney']-$sign['original_submoney'];
     		 $data['sales'] =$subinfo['sales']- $sign['original_sales'];
      	 D('Activitysign')->where(array('user_id' => $this->uid, 'activity_id' => $activity_id))->save($data);
