...
<div class="topTwo">
    <div class="left">
        <?php if(!empty($CONFIG['site']['logo'])): if(!empty($city['photo'])): ?><h1><a href="<?php echo u('pchome/index/index');?>"><img src="<?php echo config_img($city['photo']);?>" width="65" height="65" /></a></h1>
            <?php else: ?>
            <h1><a href="<?php echo u('pchome/index/index');?>"><img src="<?php echo config_img($CONFIG['site']['logo']);?>" width="65" height="65" /></a></h1><?php endif; endif; ?>
...
<div class="clear"></div>
<script type="text/javascript" src="/themes/default/Pchome/statics/js/jquery.SuperSlide.2.1.1.js"></script>
...
<script type="text/javascript">
    ...
    $(function () {
		...
		//首页活动部分结
        $(".sy_coupon_tab a").each(function (e) {
            $(this).mouseover(function (event) {
                $(this).parent().find("a").removeClass("on");
                $(this).addClass("on");
                $(".sy_coupon_list_box .sy_coupon_list").each(function (i) {
                    if (e == i){
                        $(this).parent().find(".sy_coupon_list").hide();
                        $(this).show();
                    }
                    else {
                        $(this).hide();
    ...
    <?php if(empty($MEMBER)): ?><!--未登录-->
            ...
            <div class="loginbtnboxx">
               ...
               <a href="javascript:void(0);" class="login_btndz loginbtnicon"><i></i><span>登录</span></a>
                ...
                <a href="<?php echo U('pchome/passport/register');?>" class="login_btndz registeredbtnicon"><i></i><span>注册</span></a>
        ...
        <div class="clearfix">
            ...
            <div class="ghlsmesstab">
                    ...
                    <div class="clearfix messmartb">
                        ...
                        <div class="fl insupply_box">
                            ...
                            <a target="_blank" href="<?php echo U('member/money/money');?>" class="index_supplyop">充值</a>
                        ...
                        <div class="fl instore_box">
                            ...
                            <a target="_blank" href="<?php echo U('member/index/index');?>" class="index_storesop">进入</a>
            ...
            <a href="<?php echo U('pchome/shop/apply');?>"  target="_blank" class="fl">免费商家入驻</a>

                <a href="<?php echo U('delivery/login/index');?>" target="_blank" class="fl lastcb">物流入驻</a>
...
<!--首页抢购部分开始-->
    <div class="sy_FloorBt" data="top_1" id="floor1">
        <div class="left sy_FloorBtz"><span class="bt cl_1">热门抢购</span>
            <span class="sy_buy_tab">
                <?php $i=0; ?>       
                <?php if(is_array($tuancates)): foreach($tuancates as $key=>$item2): if(($item2["parent_id"]) == "0"): $i++;if($i <= 8){ ?>
                    <a target="_blank" <?php if($i == 1) echo 'class="on"'; ?> title="<?php echo ($item2["cate_name"]); ?>抢购" href="<?php echo U('tuan/index',array('cat'=>$item2['cate_id']));?>"><?php echo ($item2["cate_name"]); ?></a>
                    <?php } endif; endforeach; endif; ?>
            </span>
        </div>
                ...
                <?php  $cache = cache(array('type'=>'File','expire'=> 600)); $token = md5("Tuan,audit = 1 AND closed=0 AND city_id=$city_id AND cate_id IN ($cate_ids),600,orderby asc,sold_num desc,0,8,,"); if(!$items= $cache->get($token)){ $items = D("Tuan")->where("audit = 1 AND closed=0 AND city_id=$city_id AND cate_id IN ($cate_ids)")->order("orderby asc,sold_num desc")->limit("0,8")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li class="nearbuy_cpLi2">
                        ...
                        <script type="text/javascript" language="javascript">
 						...
 						<?php $bg_date = strtotime($item['end_date']); $today2 = strtotime($today); ?>
                            ...
                            <span class="right"><a class="sy_hottjJd" target="_blank" href="<?php echo U('tuan/detail',array('tuan_id'=>$item['tuan_id']));?>" ><?php if($item['end_date'] > $today): ?>立即抢购<?php else: ?>已结束<?php endif; ?></a></span></p>
                            ...
                            $(function () {
                                ...
                                $('#tuan_code_<?php echo ($item["tuan_id"]); ?>').qrcode(str);
                                    ...
                                    <div class="sy_sjcpDwNr" style="padding-top:10px;">
                                        <?php if($shops[$item['shop_id']]['fans_num'] == null): ?><p><?php echo ($item["views"]); ?>人正在抢购</p>
                                        <?php else: ?>
                       ...
                       <a class="radius3 sy_sjcpCzA" href="<?php echo U('shop/detail',array('shop_id'=>$item['shop_id'],'act'=>'tuan'));?>">抢购<em>&nbsp;</em></a>
        ...
        <div class="left sy_FloorBtz"><span class="bt cl_1">热卖商品</span>
            <span class="sy_mall_tab">
                ...
                <?php if(is_array($goodscates)): foreach($goodscates as $key=>$item3): if(($item3["parent_id"]) == "0"): $i++;if($i <= 10){ ?>
                    <a target="_blank" <?php if($i == 1) echo 'class="on"'; ?> title="<?php echo ($item2["cate_name"]); ?>抢购" href="<?php echo LinkTo('mall/index',$linkArr,array('cat'=>$item3['cate_id']));?>"><?php echo ($item3["cate_name"]); ?></a>
                    <?php } endif; endforeach; endif; ?>
            </span>
        </div>
 ...
 <!--首页商城部分结束-->   
  ...
  <!--首页优惠券部分开始-->
    ...
    <div class="sy_FloorBt" data="top_8" id="floor8">
        <div class="left sy_FloorBtz"><i class="ico_8"></i><span class="bt cl_8">优惠券</span>
            <span class="sy_coupon_tab">
                <?php $couponcates = D('Shopcate')->fetchAll(); $i=0; foreach($couponcates as $k=>$val){ if($val['parent_id'] == 0){ $i++; ?>   
                <a href="<?php echo U('coupon/index',array('cat'=>$val['cate_id']));?>" <?php if(($i) == "1"): ?>class="on"<?php endif; ?>><?php echo ($val["cate_name"]); ?></a>
                <?php } } ?>
            </span>
        </div>
    ...
    <div class="sy_coupon_list_box">
        <?php $couponcates = D('Shopcate')->fetchAll(); $i=0; foreach($couponcates as $k=>$val){ if($val['parent_id'] == 0){ $i++; $cates = D('Shopcate')->getChildren($val['cate_id']); $cates = join(',',$cates); ?>   
        <ul class="sy_coupon_list" <?php if(($i) == "1"): ?>style="display:block;"<?php endif; ?>>
            <?php  $cache = cache(array('type'=>'File','expire'=> 600)); $token = md5("Coupon,audit=1 AND closed=0 AND city_id=$city_id AND cate_id IN ($cates),600,downloads desc,views desc,0,5,,"); if(!$items= $cache->get($token)){ $items = D("Coupon")->where("audit=1 AND closed=0 AND city_id=$city_id AND cate_id IN ($cates)")->order("downloads desc,views desc")->limit("0,5")->select(); $cache->set($token,$items); } ; $index=0; foreach($items as $item): $index++; ?><li>
